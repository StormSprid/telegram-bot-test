...
**Что умеет:**
- Мгновенно отвечает на типовые вопросы (контакты, доставка, оплата, бренды и т.д.) без обращения к LLM
- Направляет вопросы с советом или сравнением («что лучше», «посоветуй краску») напрямую в LLM с контекстом из базы знаний
- Стримит ответы в Telegram — сообщение появляется по мере генерации
- Помнит историю диалога в рамках сессии (скользящее окно)
- Защита от prompt injection и вопросов не по теме
- Ограничение частоты запросов на пользователя
- Уведомляет владельца в Telegram, если бот не смог ответить
- Дашборд с метриками в реальном времени: `GET /dashboard`
